...
const dataList = [
    {id:1, name:'chk1', text:'이용약관(필수)', isChecked:true},
    {id:2, name:'chk2', text:'개인정보 보호 동의(필수)', isChecked:false},
    {id:3, name:'chk3', text:'이벤트, 쿠폰, 프로모션 수신동의(선택)', isChecked:false}
]
const Test1 = () => {
    ...
    const changeCheck = (evt) => {
        ...
        if(name === 'all'){
            ...
            //각각의 chk1,chk2,chk3
            const tempChk = data.map(item=> item.name === name ? {
                ...item,
                isChecked:checked
            }: item)
    ...
    return (
        <div style={{margin:30}}>
            <h2>동의설정</h2>
            {/* {data.filter(item=> item.isChecked !== true).length} 확인용*/}
            <p>
